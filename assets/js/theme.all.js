...
jQuery(document).ready(function($){
    ...
    $(window).load(function() {
        ...
        $('.flexslider').each(function(){
            ...
            $(this).flexslider({
                animation: themeworks_theme.slideshow_animation,
                controlNav: true,
                manualControls: new_menu_item,
                slideshow: themeworks_theme.slideshow_autostart,
                smoothHeight: themeworks_theme.slideshow_smooth_height,
                prevText: "",
                nextText: "",
    ...
    }).blur().parents('form').find('#submitButton').click(function() {
    ...
    // init the progress button
    $('.progress-button').progressInitialize();

    // contact form
    $('.themeworks-contact-form').on('click', '#submitButton', function(e){
        e.preventDefault();
        ...
        var hidden = $('.contact-form .contactHide').val();
        if( '' == email ) { $('.contact-form .contact-email').addClass('invalid'); }
        if( '' == name ) { $('.contact-form .contact-name').addClass('invalid'); }
        if( '' == message ) { $('.contact-form .contact-message').addClass('invalid'); }

        if( '' != email && '' != name && '' != message && '' == hidden ) {
          $('.contact-form .fields').removeClass('invalid');
          $('.contact-all-required').addClass('hide');
          // set progress bar to auto increment for syncing the process time
          controlButton.progressStart();
          var data = {
              action: 'themeworks_contact',
              name: name,
              email: email,
              message: message
          };
          ...
          $.ajax({
              type: 'POST',
              url: themeworks_theme.ajaxurl,
              data: data,
              success: function(msg){
                  if('success' == msg ){
                    controlButton.removeClass('button-error').attr('data-finished', controlButton.attr('data-success'));
                    // set the progress meter to 100% and show the done text.
                    controlButton.progressFinish();
                } else {
                    controlButton.addClass('button-error').attr('data-finished', controlButton.attr('data-fail'));
                    controlButton.parent().find('input').val('');
                    // set the progress meter to 100% and show the done text.
                    controlButton.progressFinish();
                }
              }
            ...
            $('.contact-all-required').removeClass('hide');
        }
    });
    $('.contact-form .required').change(function(){
        $(this).removeClass('invalid');
    });

    // signup form
    // init the progress button
    $('.signup input.text').on('input', function() {
        $('.signup-button').removeClass('finished button-error');
    });

    // signup call on button click
    $('form').on('click', '.signup-button', function(e){
        e.preventDefault();
        var controlButton = $(this);
        // set progress bar to auto increment for syncing the process time
        controlButton.progressStart();
        ...
        var data = {
            action: 'themeworks_signup',
            firstname: $(this).prevAll('input.firstname[type=text]').val(),
            lastname: $(this).prevAll('input.lastname[type=text]').val(),
            email: $(this).prevAll('input.email[type=text]').val()
        };
